<script lang="ts">
  ...
  const calculateCompressionSettings = (targetSize: number, metadata: VideoMetadata): CompressionSettings => {
...
<div class="container mx-auto p-6 max-w-4xl">
  <div class="flex items-center justify-center mb-2">
    <h1 class="text-4xl font-bold mb-2 mr-4">{m.app_title()}</h1>
    <LanguageSelector />
  </div>
  ...
  {#if !isLoaded}
    <div class="flex items-center gap-1 justify-center mb-6">
      <Loader class="animate-spin h-5 w-5 text-primary" />
  ...
  <footer class="text-center text-sm text-muted-foreground mt-6 pb-6">
